...
class ReconTargetCapture(object):
    """Stash a clean copy of the decoded image before RecAug degrades it.

    RecAug (blur/noise/jitter) overwrites data['image'] in place, so the
    un-degraded pixels are gone by the time later transforms run. This must
    sit directly after DecodeImage in the transform list -- anywhere later
    and the "clean" copy would already be corrupted.
    """
    ...
    def __call__(self, data):
        data["recon_image"] = data["image"].copy()
        return data


class ReconTargetResize(object):
    """Resize/normalize the captured clean image the same way RecResizeImg
    normalizes the (degraded) training image, so the two are directly
    comparable under an L1 reconstruction loss. Must run after RecResizeImg
    (same image_shape) and after MultiLabelEncode, since MultiLabelEncode
    rebuilds the data dict and only 'recon_image' explicitly passed through
    survives -- resizing here means the resize logic only has to handle one
    dict shape.
    """
    ...
    def __call__(self, data):
        img = data["recon_image"]
        norm_img, _ = resize_norm_img(img, self.image_shape, self.padding)
        data["recon_image"] = norm_img
        return data
