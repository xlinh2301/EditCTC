...
class CTCLabelDecode(BaseRecLabelDecode):
    ...
    def __call__(self, preds, label=None, return_word_box=False, *args, **kwargs):
        if isinstance(preds, dict):
            # MultiHead returns {"ctc": ..., "length": ...} at inference once
            # use_length_head is on, even for configs whose PostProcess is
            # plain CTCLabelDecode (not the length-gated variant) -- unwrap
            # the same way tuple/list outputs are already unwrapped below.
            preds = preds["ctc"]
...
class CTCLengthGatedDecode(CTCLabelDecode):
    """CTC decode with a confidence-gated length correction.

    Greedy CTC decides the digit count implicitly by collapsing blanks, so a
    half-cut wheel at a crop boundary can get swallowed or doubled with no
    signal that anything went wrong. The auxiliary length head (trained
    alongside CTC, see rec_ctc_length_loss.py / rec_length_branch.py) predicts
    the count directly from the same features.

    This class only acts when the two disagree AND the length head is
    confident (>= length_conf_threshold). In that case it does not just trust
    the length head's number -- it re-decodes the same per-frame CTC
    probabilities under the constraint "collapse to exactly this many
    symbols" (dynamic program over CTC blank/repeat transitions) and keeps
    whichever single path is most probable under that constraint. If greedy
    already agrees with the predicted length, or the length head isn't
    confident, the greedy decode is left untouched -- this can only change
    outcomes on the disagreement cases, never on the ones already agreeing.
    """
    ...
    def __init__(
        ...
        length_conf_threshold=0.9,
        candidate_chars="0123456789",
        **kwargs,
    ):
        super(CTCLengthGatedDecode, self).__init__(
            character_dict_path, use_space_char
        )
        self.length_conf_threshold = length_conf_threshold
        # The constrained DP below is O(T * target_len * |candidates|^2); the
        # underlying head carries the FULL PP-OCRv6 dictionary (thousands of
        # classes, for pretrained-weight compatibility) even though this task
        # only ever emits digits. Running the DP over the whole dictionary is
        # not just wasteful, it is computationally infeasible (billions of
        # ops per sample -- this hung an actual training job for 30+ minutes
        # before being caught). Restricting candidates to blank + the digits
        # is both correct (no other class can be a valid meter reading) and
        # brings the DP back to the small state space it was designed for.
        self.candidate_ids = [0] + [
            i for i, c in enumerate(self.character) if c in candidate_chars
        ]

    def _constrained_decode(self, prob, target_len):
        """Best CTC path whose blank-collapsed output has exactly target_len symbols.

        prob: (T, C) frame-wise softmax probabilities, index 0 = blank.
        Returns (list_of_symbol_indices, log_prob) or (None, None) if no path
        of that length exists (e.g. target_len > T).
        """
        T = prob.shape[0]
        if target_len <= 0 or target_len > T:
            return None, None
        logp = np.log(np.clip(prob, 1e-8, 1.0))

        # state key = (k symbols emitted so far, last raw frame label)
        # last raw label 0 means "blank" (or start) -- a non-blank repeat of
        # the same symbol only collapses if nothing separates it from the
        # previous frame's raw label.
        dp = {(0, 0): 0.0}
        bp = [None] * T
        for t in range(T):
            new_dp = {}
            new_bp = {}
            row = logp[t]
            for (k, r), score in dp.items():
                for s in self.candidate_ids:
                    sp = score + row[s]
                    if s == 0:
                        nk, nr = k, 0
                    elif r == s:
                    ...
                    if nk > target_len:
                        continue
                    key = (nk, nr)
                    if key not in new_dp or sp > new_dp[key]:
                        new_dp[key] = sp
                        new_bp[key] = (k, r, s)
            dp = new_dp
            bp[t] = new_bp
        ...
        best = None
        for (k, r), score in dp.items():
            if k == target_len and (best is None or score > best[0]):
                best = (score, r)
        if best is None:
            return None, None
        score, r = best
    ...
    def __call__(self, preds, label=None, *args, **kwargs):
        if isinstance(preds, dict) and "length" in preds:
            ctc_pred, length_pred = preds["ctc"], preds["length"]
        else:
            ctc_pred, length_pred = preds, None

        if isinstance(ctc_pred, (tuple, list)):
            ctc_pred = ctc_pred[-1]
        if isinstance(ctc_pred, paddle.Tensor):
            ctc_pred = ctc_pred.numpy()
        if length_pred is not None and isinstance(length_pred, paddle.Tensor):
            length_pred = length_pred.numpy()

        preds_idx = ctc_pred.argmax(axis=2)
        preds_prob = ctc_pred.max(axis=2)
        texts = self.decode(preds_idx, preds_prob, is_remove_duplicate=True)

        if length_pred is not None:
            pred_len = length_pred.argmax(axis=1)
            len_conf = length_pred.max(axis=1)
            gated = []
            for i, (txt, conf) in enumerate(texts):
                target_len = int(pred_len[i])
                if len(txt) != target_len and len_conf[i] >= self.length_conf_threshold:
                    symbols, _ = self._constrained_decode(ctc_pred[i], target_len)
                    if symbols is not None:
                        new_txt = "".join(self.character[s] for s in symbols)
                        gated.append((new_txt, conf))
                        continue
                gated.append((txt, conf))
            texts = gated
...
class CTCCountAwareDecode(CTCLengthGatedDecode):
    """CTC decode rescored by an auxiliary count/length model (shallow fusion).

    Instead of a hard gate that either trusts greedy CTC completely or swaps
    in a length-constrained re-decode, this keeps the top-K CTC prefix beams
    (standard CTC prefix beam search, restricted to the same blank+digit
    candidate set as CTCLengthGatedDecode._constrained_decode for
    tractability) and rescores each by

        score(beam) = log P_ctc(beam) + count_beta * log P_len(len(beam))

    where P_len comes from the auxiliary length head's softmax distribution.
    This subsumes the confidence-gated design: a very large count_beta
    recovers something close to always trusting the length head, count_beta=0
    recovers plain best-of-beam CTC decoding, and everything in between is a
    smooth trade-off with no hard threshold to tune.
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super(CTCCountAwareDecode, self).__init__(
            character_dict_path,
            use_space_char,
            length_conf_threshold=kwargs.pop("length_conf_threshold", 0.99),
    ...
    def _prefix_beam_search(self, prob):
        """Standard CTC prefix beam search (Hannun et al.), restricted to
        self.candidate_ids for tractability, same restriction as the parent
        class's constrained DP.

        prob: (T, C) frame-wise softmax probabilities, index 0 = blank.
        Returns list of (prefix_symbol_tuple, log_prob) sorted best-first,
        length <= self.beam_width.
        """
        T = prob.shape[0]
        logp = np.log(np.clip(prob, 1e-8, 1.0))
        NEG_INF = -1e9

        # beams: dict prefix(tuple of symbol ids, no blanks/repeats) ->
        # (log p ending in blank, log p ending in non-blank)
        beams = {(): (0.0, NEG_INF)}
        ...
        for t in range(T):
            ...
            def add(prefix, p_b, p_nb):
                if prefix in new_beams:
                    ob, onb = new_beams[prefix]
                    new_beams[prefix] = (logsumexp(ob, p_b), logsumexp(onb, p_nb))
                else:
            ...
            for prefix, (p_b, p_nb) in beams.items():
                ...
                # extend with blank: prefix unchanged, mass moves to p_blank
                add(prefix, p_total + row[0], NEG_INF)
                for s in self.candidate_ids:
                    if s == 0:
                        continue
                    if s == last:
                        # repeat of last symbol: if it comes right after a
                        # blank (p_b), the blank has separated the repeats so
                        # this genuinely extends the prefix; if it comes
                        # right after the same non-blank symbol (p_nb), CTC
                        # collapses it -- the prefix stays the same.
                        add(prefix + (s,), NEG_INF, p_b + row[s])
                        add(prefix, NEG_INF, p_nb + row[s])
                    else:
                        add(prefix + (s,), NEG_INF, p_total + row[s])

            # prune to top beam_width by total probability
            scored = [
                (pfx, logsumexp(pb, pnb)) for pfx, (pb, pnb) in new_beams.items()
            ]
            scored.sort(key=lambda x: x[1], reverse=True)
            keep = scored[: self.beam_width]
            beams = {pfx: new_beams[pfx] for pfx, _ in keep}

        final = [(pfx, logsumexp(pb, pnb)) for pfx, (pb, pnb) in beams.items()]
    ...
    def __call__(self, preds, label=None, *args, **kwargs):
        ...
        if isinstance(preds, dict) and "length" in preds:
            ctc_pred, length_pred = preds["ctc"], preds["length"]
            if "count_beta" in preds:
                learned_beta = preds["count_beta"]
                if isinstance(learned_beta, paddle.Tensor):
                    learned_beta = float(learned_beta.numpy().reshape([-1])[0])
        else:
            ctc_pred, length_pred = preds, None
        ...
        # A learned count_beta (from LengthCountFusionLoss training) takes
        # precedence over the static config value when present.
        beta = learned_beta if learned_beta is not None else self.count_beta

        preds_idx = ctc_pred.argmax(axis=2)
        preds_prob = ctc_pred.max(axis=2)
        greedy_texts = self.decode(preds_idx, preds_prob, is_remove_duplicate=True)

        if length_pred is None:
            if label is None:
                return greedy_texts
            return greedy_texts, self.decode(label)

        results = []
        for i, (greedy_txt, greedy_conf) in enumerate(greedy_texts):
            log_p_len = np.log(np.clip(length_pred[i], 1e-8, 1.0))
            beams = self._prefix_beam_search(ctc_pred[i])
            if not beams:
                results.append((greedy_txt, greedy_conf))
                continue
            best_txt, best_conf, best_score = None, None, None
            for symbols, log_p_ctc in beams:
                n = len(symbols)
                bonus = log_p_len[n] if n < len(log_p_len) else -1e9
                score = log_p_ctc + beta * bonus
                if best_score is None or score > best_score:
                    best_score = score
                    best_txt = "".join(self.character[s] for s in symbols)
                    best_conf = greedy_conf
            results.append((best_txt, best_conf))

        if label is None:
            return results
        return results, self.decode(label)
