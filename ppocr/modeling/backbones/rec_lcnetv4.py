...
class PPLCNetV4(nn.Layer):
    ...
    def forward(self, x):
        if self.det:
            ...
            x = self.blocks5(x)
            x = self.blocks6(x)
            # captured before the final height-collapsing pool below: the
            # eval branch asserts height >= 3 at this point, so this is the
            # last feature map with any real vertical structure left. An
            # optional reconstruction head reads this via backbone.recon_feat
            # instead of the fully height-pooled output the recognizer uses.
            self.recon_feat = x
            if self.training:
