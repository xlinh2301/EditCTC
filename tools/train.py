...
def main(config, device, logger, vdl_writer):
    ...
    # Additive: optionally partial-load a second checkpoint into a submodule
    # after the base pretrained_model has loaded (e.g. a text-only
    # pretrained EditRefineDecoder). Reuses the existing
    # load_pretrained_params partial-load behavior (keys not present in the
    # full model's state_dict, or with a mismatched shape, are skipped with
    # a warning; matching keys load normally) -- no new loading code needed.
    extra_pretrained = config["Global"].get("edit_refine_pretrained")
    if extra_pretrained:
        from ppocr.utils.save_load import load_pretrained_params

        logger.info(
            "Partial-loading extra pretrained submodule weights from {}".format(
                extra_pretrained
            )
        )
        load_pretrained_params(model, extra_pretrained)

    # Additive (Loop B, direction #1 two-stage): freeze every parameter except
    # the edit-refinement head. Stage-2 trains only the edit decoder on top of a
    # converged, FROZEN CTC backbone -> the CTC seed is now stationary across
    ...
    if config["Global"].get("freeze_except_edit"):
        ...
        for _name, _p in model.named_parameters():
            if "edit_refine" in _name:
                _p.stop_gradient = False
                n_train += 1
            else:
                _p.stop_gradient = True
                n_frozen += 1
        logger.info("freeze_except_edit: froze {} params, training {} edit-head params".format(n_frozen, n_train))
